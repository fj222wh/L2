...
| ----- | -------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | `createInvoice()`                                  | A method in the Order class to create the invoice.                   | **Intention revealing name**, **Add meaningful context**, **easy to pronunce**, I think this name is pretty short and easy to pronunce as well to understand what it does. It doesn't really give the reader understanding of what format the invoice is, e.g. that it will be returned in HTML in this case, but it gives the reader enough context on what the method's purpose is at least.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **2** | `calculateTotalPrice ()`                           | A method in the Order class to calculate the total price.            | **Intention revealing name, meaningful context, Pronounceable names, avoid, disinformation, searchable name**. This method name gives clear context on what the method does, it is also pretty short, which makes the readability high amd it's easy search up this name while coding.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **3** | `updateOrderItemQuantity (productId, newQuantity)` | A method in the Order class to update the quantity of an order item. | **Problem domain name, meaningful context, searchable name** this name is from problem domain name because it uses the types of names we would use in the business where this code will be used. It's easy to understand for non-programmers what it does, and how to use it. The name and its parameters are a bit longer, but still gives the reader enough information about what the method and what the parameters mean. The reader doesn't have to read all the documentation to understand what this method does. It's a bit longer, but it's still pronounceable. The names are also searchable, names are very specific and it would be harder to mix them up with something else. Not trying to be cute only to describe what method and what parameters mean. |
| **4** | `isActiveOrder ()`                                 | A method to check if an order is active. Returns a boolean           | **Intention revealing name, high readability, Pronounceable names** With the name starting with "is", it shows that the method returns a boolean weather the order is active or not. This name intention, revealing because clearly says what the method is looking for while keeping the high readability.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **5** |  `ProductCatalog`                                                  | Class name for the class handling the product catalog                | **One Word per Concept, Don't be cute, Pronounceable names**. I chose this name because it's straightforward, short and searchable. with this class name reader understands what this class does, that it contains the product catalog and its methods. I didn't try to be cute or come up with him more fun name, I try to keep it very simple.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
```
I think the names in this function and the parameters that has been used, gives a clearer context to what the method does. By only reading the method name and the parameters you can understand what the method does without reading the code. Although the names could have been shorter to improve the readability, but then they would lose some context. In this case, for example I changed the method name to only updateQuantity, it might be able to understand what the method anyways since this is a public method in the class Order, but I think the longer name gives a better context.

When I wrote the code, I tried to think about giving the variables a good name from the start, but it was sometimes hard. I noticed that sometimes I had given a variabel a bad name, because when I came back to the code after a few days, I couldn't understand what the code was doing without reading the entire function - then I realized the name wasn't good enough.

Changing the names after already declearing and using them in both the code and tests were a bit of a hustle. The code broke several times for me when I had changed the name because in the code the methods were still calling the old name. Sometimes it didn't work to use the button "rename symbal", so I had to manually look and seacrh for the variable names. I'm happy I had the testcases, it made it easier to find where something went wrong after I changed the name. I tried to create tests with as high coverage as possible and therefore my test were pretty good at finding bugs where the code stopped working because of the changed name.

I think the most of the rules in this book about the names are logic, for example keeping the names readable by not using way too long variable names, including enough context with names and not make the readers confused about what the variables mean.

There are a few times where I changed the name to making more logic or keep the naming consistent through all of the classes. Sometimes I only kept the names consistent within its own class, and not consistent through the module. For example I changed so it never said display something since it doesn't displays anything. For example I had a method for getting all of the products in the product catalog, the return value is just an array and a method itself isn't really displaying anything. That's why it chose to use the word get instead. I think the word display would be more suitable if this was a method in the frontend or in the UI.

When I store the information about what products that exist in the cart, I save an object refering to the orderItem (I probably should have created a class instead for this, to be more clear but I realised it too late), therefore I changed several names from `product` to `orderItem` instead when naming variables and methods in the Order class. I thought a name could be a bit misleading since I didn't actually save an instance of the product.

Example: I changed this to find OrderItem instead of findProduct in the orderClass. Similar with the removeProductById was changed to removeOrderItem.
...
About the code doing one thing - is it wrong that I update the quantity if the orderItem already exists in the cart? I don't know. I was thinking about if the user add's an item and it's already in the cart I just want to increase the quantity but I'm a bit unsure if this counts as doing two things? The code adds the orderItem to cart either way so therefore I thought this was doing one thing.
...
I broke the has no side affects and that the code should only do one thing, but I tried to refactor my code to make sure the code doesn't have side affects and that it only does one thing. I understand the benefits of few parameters although on some places I had already made a bad design, and I didn't know how to refactor it in a good way during the given time. But in the future, I will have to refactor it a bit more to remove some of the parameters to make the code easier to use and understand.
...
Overall I tried to keep the code as clean as possible althogh there of course are room for improvement.
